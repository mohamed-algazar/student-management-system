...
module Calculations =
    ...
    let getStudentGradeSummary (student: Student) : (float * float * float * float) =
        ...
        let total = student.Grades |> List.sum
        let highest = match student.Grades with | [] -> 0.0 | grades -> List.max grades
        let lowest = match student.Grades with | [] -> 0.0 | grades -> List.min grades
...
type Msg =
    ...
    | UpdateNewStudentGrades of string
    | AddNewStudent
...
let init () =
    let sampleStudents = [
        { Id = 1; Name = "Alice Johnson"; Age = 20; Grades = [85.5; 92.0; 88.5]; Class = "CS101" }
        { Id = 2; Name = "Bob Smith"; Age = 19; Grades = [78.0; 82.5; 80.0]; Class = "CS101" }
        { Id = 3; Name = "Carol White"; Age = 21; Grades = [95.0; 98.5; 96.0]; Class = "CS102" }
        { Id = 4; Name = "David Brown"; Age = 20; Grades = [55.0; 62.0; 58.5]; Class = "CS101" }
    ]
    {
        ...
        NewStudent = { Id = 0; Name = ""; Age = 18; Grades = []; Class = "" }
        EditingStudent = None
...
let update (msg: Msg) (model: Model) =
    ...
    | ShowAddStudentDialog ->
        { model with ShowAddDialog = true; NewStudent = { Id = 0; Name = ""; Age = 18; Grades = []; Class = "" } }, Cmd.none
    
    | HideAddStudentDialog ->
        { model with ShowAddDialog = false; NewStudent = { Id = 0; Name = ""; Age = 18; Grades = []; Class = "" } }, Cmd.none
    
    | ShowEditStudentDialog student ->
        { model with ShowEditDialog = true; EditingStudent = Some student; NewStudent = student }, Cmd.none
    ...
    | UpdateNewStudentGrades gradesStr ->
        let grades = 
            gradesStr.Split(',')
            |> Array.choose (fun s -> 
                ...
                | _ -> None)
            |> Array.toList
        { model with NewStudent = { model.NewStudent with Grades = grades } }, Cmd.none
    ...
    | AddNewStudent ->
        if Auth.canPerformOperation model.System "add" then
            ...
            { model with System = newSystem; CurrentMessage = Some message; ShowAddDialog = false }, Cmd.none
        else
    ...
    | UpdateExistingStudent ->
        if Auth.canPerformOperation model.System "update" then
            ...
            | Some originalStudent ->
                ...
                { model with System = newSystem; CurrentMessage = Some message; ShowEditDialog = false; EditingStudent = None }, Cmd.none
            | None ->
            ...
            Html.div [
                ...
                prop.children [
                    Html.p [
                        ...
                        prop.text $"Grades: {String.concat ", " (student.Grades |> List.map (sprintf "%.1f"))}"
                    ]
                ]
            ]
        ]
...
let private studentDialog model dispatch isEdit =
    if not (if isEdit then model.ShowEditDialog else model.ShowAddDialog) then Html.none
        ...
        let gradesStr = String.concat ", " (model.NewStudent.Grades |> List.map string)
        
        Html.div [
            prop.className "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
            prop.children [
                Html.div [
                    prop.className "bg-white rounded-xl shadow-2xl p-8 w-96 max-w-full"
                    prop.children [
                        Html.h2 [
                        ...
                        Html.div [
                            ...
                            prop.children [
                                ...
                                Html.input [
                                    prop.className "w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                    prop.placeholder "Grades (comma-separated, e.g., 85.5, 92.0, 88.5)"
                                    prop.value gradesStr
                                    prop.onChange (fun (v: string) -> dispatch (UpdateNewStudentGrades v))
                                ]
                            ]
                        ]
                        Html.div [
